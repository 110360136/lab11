    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btn_query.setOnClickListener(view -> {
            ...
            if (ed_book.length() <1 )
                ...
                c = dbrw.rawQuery( "SELECT * FROM myTable WHERE bool LIKE '"+ed_book.getText().toString()+ "'",  null);

            c.moveToFirst();
            items.clear();
            Toast.makeText( MainActivity.this, "共有" + c.getCount() + "筆",
                            Toast.LENGTH_SHORT).show();
             for (int i = 0; i < c.getCount(); i++){
                 items.add("書籍" + c.getCount(0))
             }
